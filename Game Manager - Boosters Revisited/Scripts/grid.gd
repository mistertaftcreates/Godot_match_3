...
export (PoolVector2Array) var swirl_spaces
...
func spawn_swirl():
	if swirl_spaces != null:
		if swirl_spaces.size() > 0:
			for i in swirl_spaces.size():
				var temp = load("res://Scenes/LicoriceSwirl.tscn").instance()
				add_child(temp)
				temp.position = grid_to_pixel(swirl_spaces[i].x, swirl_spaces[i].y)
				all_pieces[swirl_spaces[i].x][swirl_spaces[i].y] = temp
...
func swap_back():
	...
	if piece_one != null && piece_two != null && !match_found:
...
func find_matches(query = false, array = all_pieces):
	for i in width:
		for j in height:
			if array[i][j] != null and array[i][j].is_matchable:
	...
	if !color_bomb_used:
		...
		for i in current_matches.size():
			...
			for j in current_matches.size():
				...
				#var this_color = all_pieces[current_matches[j].color
...
func destroy_matched():
	...
	for i in width:
		for j in height:
			if all_pieces[i][j] != null:
				if all_pieces[i][j].matched:
					if all_pieces[i][j].color != "NotMatched":
						damage_special(i, j)
						was_matched = true
						match_found = true
	for i in width:
	...
	elif !was_matched and !match_found:
		swap_back()
		print("no matches")
...
func check_swirls(column, row):
	var swirl_damaged = false
	# Check Right
	if column < width - 1:
		swirl_damaged = damage_swirl(Vector2(column + 1, row), swirl_damaged)
	# Check Left
	if column > 0:
		swirl_damaged = damage_swirl(Vector2(column - 1, row), swirl_damaged)
	# Check up
	if row < height - 1:
		swirl_damaged = damage_swirl(Vector2(column, row + 1), swirl_damaged)
	# Check Down
	if row > 0:
		swirl_damaged = damage_swirl(Vector2(column, row - 1), swirl_damaged)
	#if swirl_damaged:
		#destroy_licorice_swirl()
		#destroy_matched()


func damage_swirl(position, swirl_damaged):
	if all_pieces[position.x][position.y] != null:
		if !all_pieces[position.x][position.y].is_matchable:
			all_pieces[position.x][position.y].damage(1)
			swirl_damaged = true
	return swirl_damaged


func destroy_licorice_swirl():
	for i in width:
		for j in height:
			if all_pieces[i][j] != null:
				if all_pieces[i][j].matched and all_pieces[i][j].color == "NotMatched":
					all_pieces[i][j].queue_free()
					all_pieces[i][j] = null
		...
		for j in height:
			if all_pieces[i][j] != null:
				if match_at(i, j, all_pieces[i][j].color) or all_pieces[i][j].matched:
					find_matches()
					print("checking from after refill")
					get_parent().get_node("destroy_timer").start()
					return
	if !damaged_slime:
		generate_slime()
