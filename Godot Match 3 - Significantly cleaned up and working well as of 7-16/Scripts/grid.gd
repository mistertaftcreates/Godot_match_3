...
var state
signal broadcast_state
signal check_win

# to replace state machine:
var can_move = false
...
# Obstacle Signals
signal make_obstacle
signal damage_obstacle
...
func _ready():
	can_move = true
	emit_signal("broadcast_state", true)
	randomize();
	move_camera()
...
func spawn_ice():
	if ice_spaces != null:
		for i in ice_spaces.size():
			emit_signal("make_obstacle", ice_spaces[i], "ice")

func spawn_locks():
	if lock_spaces != null:
		for i in lock_spaces.size():
			emit_signal("make_obstacle", lock_spaces[i], "lock")

func spawn_concrete():
	if concrete_spaces != null:
		for i in concrete_spaces.size():
			emit_signal("make_obstacle", concrete_spaces[i], "concrete")

func spawn_slime():
	if slime_spaces != null:
		for i in slime_spaces.size():
			emit_signal("make_obstacle", slime_spaces[i], "slime")
...
func swap_pieces(column, row, direction):
	...
	if first_piece != null && other_piece != null:
		if !restricted_move(Vector2(column, row)) and !restricted_move(Vector2(column, row) + direction):
			...
			can_move = false
			emit_signal("broadcast_state", can_move)
...
func check_concrete(column, row):
	# Check Right
	if column < width - 1:
		emit_signal("damage_obstacle", Vector2(column + 1, row), "concrete")
	# Check Left
	if column > 0:
		emit_signal("damage_obstacle", Vector2(column - 1, row), "concrete")
	# Check up
	if row < height - 1:
		emit_signal("damage_obstacle", Vector2(column, row + 1), "concrete")
	# Check Down
	if row > 0:
		emit_signal("damage_obstacle", Vector2(column, row - 1), "concrete")
...
func check_slime(column, row):
	# Check Right
	if column < width - 1:
		emit_signal("damage_obstacle", Vector2(column + 1, row), "slime")
	# Check Left
	if column > 0:
		emit_signal("damage_obstacle", Vector2(column - 1, row), "slime")
	# Check up
	if row < height - 1:
		emit_signal("damage_obstacle", Vector2(column, row + 1), "slime")
	# Check Down
	if row > 0:
		emit_signal("damage_obstacle", Vector2(column, row - 1), "slime")

func damage_special(column, row):
	emit_signal("damage_obstacle", Vector2(column,row), "ice")
	emit_signal("damage_obstacle", Vector2(column,row), "lock")
	...
	if slime_spaces.size() > 0:
		...
		while !slime_made and tracker < 100:
			...
			if neighbor != null:
				...
				emit_signal("make_obstacle", Vector2(neighbor.x, neighbor.y), "slime")
				slime_made = true
			tracker += 1
